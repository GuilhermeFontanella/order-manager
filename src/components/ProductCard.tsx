...
export default function ProductCard({ item, onAdd, showDetails }: Props) {
  return (
    <motion.div
      layout
    ...
    >
      <div onClick={() => showDetails(item)} className="w-12 h-12 rounded-md bg-gray-100 flex items-center justify-center text-xl" aria-hidden>
        {item.emoji ?? '🍽️'}
      </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex sm:flex-col md:flex-col gap-3 lg:flex-row sm:items-start justify-between">
          ...
          <div className="flex items-center gap-3 sm:justify-end">
            <div className="font-mono font-semibold text-sm text-emerald-700 text-nowrap">{fmt(item.preco)}</div>
                <button
                onClick={() => onAdd?.(item)}
                className=" flex justify-center items-center  rounded-full bg-emerald-700 text-white shadow-lg shadow-emerald-200/60 transition hover:bg-emerald-800"
